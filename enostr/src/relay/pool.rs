...
use crate::relay::nip11::{fetch_relay_info, RelayInfo};
...
pub struct RelayPool {
    ...
    /// NIP-11 relay metadata cache (URL -> RelayInfo)
    pub relay_info: HashMap<String, RelayInfo>,
    /// Whether to fetch NIP-11 metadata
...
impl RelayPool {
    ...
    pub fn new() -> RelayPool {
        RelayPool {
            ...
            enable_nip11: true, // Enabled by default
        }
    }

    /// Enable or disable NIP-11 relay metadata fetching
    pub fn with_nip11(mut self, enable: bool) -> Self {
        self.enable_nip11 = enable;
        self
    }

    /// Fetch NIP-11 metadata for all relays in the pool
    ///
    /// This queries each relay's HTTP endpoint in parallel to discover:
    /// - max_limit (optimize subscription sizes)
    /// - payment_required, auth_required
    /// - supported_nips
    /// - retention policies
    ///
    /// Results are cached in `relay_info` HashMap.
    pub async fn fetch_relay_metadata(&mut self) {
        if !self.enable_nip11 {
            debug!("NIP-11 metadata fetching disabled");
            return;
        }

        info!("Fetching NIP-11 metadata for {} relays...", self.relays.len());

        // Fetch all relays in parallel
        let mut handles = Vec::new();
        let urls: Vec<String> = self.relays.iter().map(|r| r.url().to_string()).collect();

        for url in urls {
            let url_clone = url.clone();
            let handle = tokio::spawn(async move {
                match fetch_relay_info(&url_clone).await {
                    Ok(info) => {
                        info!(
                            "✓ {}: max_limit={:?}, payment={}, auth={}",
                            url_clone,
                            info.max_limit(),
                    ...
                    Err(e) => {
                        warn!("✗ {}: NIP-11 fetch failed: {}", url_clone, e);
                        None
                    }
                }
            });
        ...
        for handle in handles {
            if let Ok(Some((url, info))) = handle.await {
                self.relay_info.insert(url, info);
            }
        }

        info!(
            "NIP-11 metadata fetched for {}/{} relays",
            self.relay_info.len(),
            self.relays.len()
        );
    }
    ...
    pub fn get_relay_info(&self, url: &str) -> Option<&RelayInfo> {
        self.relay_info.get(url)
    }

    /// Optimize filter limit based on relay's max_limit
    ///
    /// If relay advertises max_limit via NIP-11, clamps the requested
    /// limit to avoid silent truncation by the relay.
    pub fn optimize_limit(&self, relay_url: &str, requested_limit: u32) -> u32 {
        match self.get_relay_info(relay_url) {
            Some(info) => {
                let optimized = info.optimize_limit(requested_limit);
                if optimized < requested_limit {
                    debug!(
                        "{}: Clamping limit {} → {} (relay max_limit: {:?})",
                        relay_url,
                        requested_limit,
                        optimized,
                        info.max_limit()
            ...
            None => {
                // No NIP-11 info available, use requested limit as-is
                requested_limit
            }
        }
    }
